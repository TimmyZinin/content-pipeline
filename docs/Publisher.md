...
> - Curator ставит `approved` (не `scheduled`). Публикация требует `/approve` gate.
> - Публикация идёт только для rows со статусом `scheduled`
> - Для актуального snapshot: `GET /queue` или `SELECT status, count(*) FROM content.platform_posts GROUP BY status`
...
**Статус платформ:** см. таблицу ниже (секция "Текущий статус платформ") и docs/Platforms.md.

Все платформы проходят через Quality Gate перед публикацией.

### Текущий статус платформ (1 апр 2026)

> Актуальные даты — см. docs/Platforms.md

| Платформа | Статус | Last verified | Adapter |
|-----------|--------|--------------|---------|
| Telegram | ⚠️ Recently | Mar 31 | telegram.py |
| Threads RU | ⚠️ Recently | Mar 30 | threads.py |
| Bluesky | ⚠️ Recently | Mar 30 | bluesky.py (auto-resize) |
| Mastodon | ⚠️ Recently | Mar 29 | mastodon.py |
| Threads EN | ⚠️ Recently | Mar 28 | Publer |
| Facebook | ⚠️ Recently | Mar 28 | Publer |
| VK | ⚠️ Recently | Mar 28 | vk.py |
| LinkedIn | ⚠️ Recently | Mar 27 | linkedin.py |
| Dev.to | ⏳ Historic | Mar 23 | devto.py |
| Hashnode | ⏳ Historic | Mar 23 | hashnode.py |
| Write.as | ⏳ Historic | Mar 22 | writeas.py |
| Minds | ⏳ Historic | Mar 22 | minds.py |
| Nostr | ⏳ Historic | Mar 22 | nostr.py |
| Tumblr | ❌ Blocked | Never | tumblr.py (401) |
...
## Архив: Sprint 4B (выполнен)

> Бэклог Sprint 4B закрыт. Bluesky/Mastodon/LinkedIn/Publer platforms — все работают.
> Tumblr — blocked (401 OAuth), не в allowlist.
